...
class AMP{
    ...
    */ 
   template <typename R>
    AMP(const T &number_of_sections=8, const T & section_size=2, const R& power_allocation):L(number_of_sections), B(section_size){
        this->code_messgae.resize(B * L, 0); // Setting the size of the code
        gen_sparse_code(power_allocation); // Generate the sparse code randomly
    }
    template <typename R>
...
void AMP<T>::gen_sparse_code(const R& power_allocation){
    ...
    for(int i=0;i < this->L; i++){
        ...
        int j = dist(gen);
        this->code_messgae[i * this->B + j] = 1; 
    }  
}
#endif // AMP_H