...
public:
    ...
    */
   std::vector<double> compute_matrix_vector_multiplication(const std::vector<double> &M, const std::vector<double>&V);
private:
...
void AMP::get_decoded_message(const double &snr, const double &rate, const int &t_max, const double &error_threshold, const double &power_allocation)
{
    ...
    while (t < t_max)
    {
        ...
        std::vector<double> F_square(F_copy); 
        // Compute F**2 
        std::transform(
            F_square.begin(), 
            F_square.end(), 
...
/*
    Compute Matrix-Vector multiplication
*/
std::vector<double> AMP::compute_matrix_vector_multiplication(const std::vector<double> &M, const std::vector<double>&V){
        int m = V.size(); 
        int n = M.size() / m; 
