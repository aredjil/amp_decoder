...
public:
    ...
    */
    ...
    /*
        Compute z
    */
    std::vector<double> compute_z(const std::vector<double> &F, const std::vector<double> &x_hat, const std::vector<double> &codeword, const std::vector<double> &V, const std::vector<double> &V_old, const std::vector<double> &z_old, const double &snr);
...
void AMP::get_decoded_message(const double &snr, const double &rate, const int &t_max, const double &error_threshold, const double &power_allocation)
{
    ...
    std::vector<double> x_hat(this->N, 0.0);
    std::vector<double> v(this->N, 1.0 / (snr * this->B));

    std::vector<double> F_copy(this->F);
    std::vector<double> F_square(this->F);
    ...
    while (t < t_max)
    {
        ...
        std::vector<double> x_hat_old(x_hat);

        // Computing the Matrix-Vector multiplication V = F**2.v
        V = compute_matrix_vector_multiplication(F_square, v);
        z = compute_z(F, x_hat, codeword, V, V_old, z_old, snr);
        ++t;
    }
}
/*
    Compute Matrix-Vector multiplication
*/
std::vector<double> AMP::compute_matrix_vector_multiplication(const std::vector<double> &Mat, const std::vector<double> &Vec)
{
    int m = Vec.size();
    int n = Mat.size() / m;
...
/*
    Compute z
*/
std::vector<double> AMP::compute_z(const std::vector<double> &F, const std::vector<double> &x_hat, const std::vector<double> &codeword, const std::vector<double> &V, const std::vector<double> &V_old, const std::vector<double> &z_old, const double &snr){

    std::vector<double> buffer_Fx_hat(this->M);
    std::vector<double> buffer_frac(this->M);
    std::
    // Compute F.x_hat 
    buffer_Fx_hat = compute_matrix_vector_multiplication(F, x_hat);
    // Compute the frac between V_old and current V
    std::transform(
        V_old.begin(), 
        V_old.end(), 
        V.begin(), 
        buffer_frac.begin(), 
        [snr](const double &a, const double & b){
            return a/(b+(1.0/snr));
        }
    );
