...
void AMP::get_decoded_message(const double &snr, const double &rate, const int &t_max, const double &error_threshold, const double &power_allocation)
{
    ...
    while (t < t_max)
    {
        ...
        std::vector<double> Fxhat(this->M, 0.0);
        std::vector<double> onsager_term(this->M, 0.0);
        // Computing the Matrix-Vector multiplication V = F**2.v
        V = this->compute_matrix_vector_multiplication(F_square, v);
        // Compute F.x_hat
        Fxhat = this->compute_matrix_vector_multiplication(F, x_hat);

        onsager_term = compute_onsager_term(V, V_old, z_old, snr);
        ++t;
    }
}
/*
...
/*
    Copmpute
*/
std::vector<double> AMP::compute_onsager_term(const std::vector<double> &V, const std::vector<double> &V_old, const std::vector<double> &z_old, const double &snr)
{

    std::vector<double> buff_one(codeword.size());
    std::vector<double> buffer_two(V.size());

    std::vector<double> buffer_result(z_old.size()); 
// Compute V_old /(1 / snr + V)
    std::transform(
        V_old.begin(),
        V_old.end(),
        V.begin(),
        buff_one.begin(),
        [snr](const double &v_old, const double &v)
        {
            return v_old / (v + (1.0 / snr));
        });
// Compute y - z  
    std::transform(
        codeword.begin(),
        ...
        z_old.begin(),
        buffer_two.begin(),
        [snr](const double &y, const double &z)
        {
            return (y-z);
        }
    );
