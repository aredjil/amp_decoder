...
public:
    ...
    */
    ...
    AMP(const int &number_of_sections, const int &section_size, const double &power_allocation)
        : L(number_of_sections) // NUmber of sections
          ,
          B(section_size) // Section size
          ,
          c(power_allocation), gen(dv()) // Random number generator
    {
...
void AMP::gen_sparse_code()
{
    ...
    const double upper_bound = static_cast<double>(this->B) -1.0;
    std::uniform_real_distribution<int> dist(lower_bound, upper_bound);
