...
public:
    ...
    */
    ...
    void compute_V(std::vector<double> &F_2, std::vector<double> &v, std::vector<double> &V);
...
*/ 
void AMP::compute_V(std::vector<double> &F_2, std::vector<double> &v, std::vector<double> &V)
{
    cblas_dgemv(
        CblasRowMajor,
        CblasNoTrans,
...
void AMP::solve(){
    ...
    std::vector<double> Fa(M, 0.0);
    
    std::transform( // Compute F_{ij}**2
        F_2.begin(),
        F_2.end(), 
        F_2.begin(), 
        [](double x){ return x * x; }
    ); 
    compute_V(F_2, v, V); // Compute the first term in the algorithm 
