...
void AMP::solve(){
    ...
    std::vector<double> omega_old(codeword);
    std::vector<double> omega_old(M);
    // Compute F_{ij}**2
    std::transform( 
        F_2.begin(),
        F_2.end(), 
    ...
    // Compute omege_new 
    compute_onsegar(std::vector<double> codeword, std::vector<double> omega_old, std::vector<double> V_new, std::vector<double> V_old, std::vector<double> Fa,std::vector<double>omega_new);
