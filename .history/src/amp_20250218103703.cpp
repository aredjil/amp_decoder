...
{
    const double power_alloc = this->c; 
    std::vector<double> normlization_fact(L, 0.0);
    ...
    std::transform(
        ...
        cavity_mean.begin(), 
        exp_term.begin(), 
        [power_alloc](double r, double sigma){
            return std::exp(-power_alloc*(power_alloc-2*r)/(2 * sigma * sigma));
        }
    );
    //1. Compute the exponential term 
    // 2. Compute the normlization factor
    for (int j = 0; j < B; j++)
    {
        normlization_fact[j] =;
    }

    // 2.0 Update the new value of the estimate a_{i}^{t+1}
}
