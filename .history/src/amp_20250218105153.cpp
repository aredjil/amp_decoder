...
{   const double power_alloc = this->c; // Get the power allocation  
    ...
    for(int i=0;i<N;i++)
    {
        a_new[i] = power_alloc * (std::exp(-power_alloc*(power_alloc-2 * cavity_mean[i])/(2 * sigma_new[i]))) /exp_term[(i + B) % B];
    }
}
...
void AMP::solve()
{
    ...
    denoise_a(std::vector<double> sigma_new, std::vector<double> cavity_mean, std::vector<double> &a_new)
}