...
// Get the the value of the error of the estimation a
void denosie_v(std::vector<double> sigma_new, std::vector<double> cavity_mean, std::vector<double> &v_new);
...
void AMP::solve()
{
    ...
    denosie_v(sigma_new, cavity_mean, v_new);
    for (int i = 0; i < L; i++)
    {
        for (int j = 0; j < B; j++)
        {
