...
void AMP::compute_cavity_var(std::vector<double> F_2, std::vector<double> V_new, std::vector<double> &sigma_new)
{
    ...
    // Call cblas_dgemv to perform vector-matrix multiplication: y = alpha*x*A + beta*y
    // This is equivalent to y = alpha*(A^T)^T*x + beta*y
    // cblas_dgemv(CblasColMajor,   // Layout: column-major order
    //             CblasTrans,       // Transpose: use A^T
    //             N, M,             // Dimensions: N rows, M columns (of A^T)
    //             alpha,            // Scalar alpha
    //             F_2.data(), M,             // Matrix A with leading dimension M
    //             V_inv.data(), 1,             // Vector x with stride 1
    //             beta,             // Scalar beta
    //             sigma_new.data(), 1);            // Vector y with stride 1
    // cblas_dgemv(
    //     CblasColMajor,
    //     CblasTrans,
    //     this->N, this->M,
    ...
    for(int i=0;i<M;i++)
    {
        ...
        {   
            sum += F_2[mu*M+i] *  (snr_inv + V_inv[mu]);
        }
        sigma_new[i] = 1.0 / sum;
    }
