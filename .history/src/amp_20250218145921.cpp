...
// Name it update_V 
// only pass v_old and V_old compute F_{ij}**2 inside the function
...
{
    const data_t power_alloc = c; // Get the power allocation
...
void AMP::solve()
{
    ...
    std::vector<data_t> a_old(N, 1.0); // Store the estimation of the message
    std::vector<data_t> a_new(N, 1.0); // Store the new estimation of the message
