      ...
      snr(signal_to_noise_ration), gen(dv()) // Random number generator
{
    ...
    this->N = this->L * B;         // Setting the size of the sparse code
    this->M = (int)(this->N * std::log2(B)) / (r * B);

    this->F.resize(M * N);  // Setting the size of the coding matrix  F is N*M matrix
...
void AMP::denoise_a(std::vector<data_t> sigma_new, std::vector<data_t> cavity_mean, std::vector<data_t> &a_new)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        a_new[i] = c * (std::exp(-c * (c - 2 * cavity_mean[i]) / (2 * sigma_new[i])));
    }
}
