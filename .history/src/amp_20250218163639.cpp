...
std::vector<data_t> AMP::amplify(std::vector<double> a, std::vector<double> &a_temp)
{
    std::vector<double> a_temp(data.size(), 0.0);
    for (int i = 0; i < L; i++)
    {
        ...
        std::fill(row_begin, row_end, 0.0);

        // Set the max element to 1
        row_begin[max_idx] = 1.0;
    }
}
