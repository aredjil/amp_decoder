...
std::vector<data_t> AMP::amplify(std::vector<data_t> a)
{
    std::vector<data_t> a_temp(a.size(), 0.0);
    for (int i = 0; i < L; i++)
    {
        // Find the maximum element in the current row
        double *row_begin = a.data() + i * B;
        double *row_end = row_begin + B;
        double *max_element = std::max_element(row_begin, row_end);
