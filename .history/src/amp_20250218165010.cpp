...
std::vector<data_t> AMP::amplify(std::vector<data_t> a_new)
{
    std::vector<data_t> a_temp(a.size(), 0.0);
    for (int i = 0; i < L; i++)
    {
        // Find the maximum element in the current row
        data_t *row_begin = a.data() + i * B;
        data_t *row_end = row_begin + B;
        data_t *max_element = std::max_element(row_begin, row_end);

        // Get the index of the maximum element within the row
        int max_idx = std::distance(row_begin, max_element);

        a_temp[i * B+max_idx] = 1.0;
        // Set the max element to 1
    }
    return a_temp;
}
bool same_row(const std::vector<data_t> &a_estimate,const std::vector<data_t> &a_real)
{
    for(int i=0;i<L;i++)
    {
        for(int j=0;j<B;j++)
...
data_t AMP::compute_ser(std::vector<data_t> a_new)
{
    data_t sum = 0.0; 
...
void AMP::solve()
{
    ...
    std::vector<data_t> a_temp(N, 0.0); // Store the new estimation of the message
