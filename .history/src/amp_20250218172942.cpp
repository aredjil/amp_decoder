...
data_t AMP::compute_dif(const std::vector<data_t> &a_new, const std::vector<data_t> &a_old, data_t &diff)
{
    data_t sum =0.0;
    for (int i = 0; i < N; i++)
    {
...
data_t AMP::compute_mse(const std::vector<data_t>& a_new, data_t &mse_err)
{
    data_t err =0.0; 
    for(int i=0;i<N;i++)
    {
