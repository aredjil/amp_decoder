...
data_t AMP::compute_ser(const std::vector<data_t> &a_temp)
{
    ...
    int count = 0; // Varible to hold the count sections that were correctly reconstructed 
     // Counter to track the number of enteries 
                     // Correctly reconstructed per section  
    for(int i = 0;i<L;i++)
    {
        int id_count = 0;
        for(int j=0;j<B;j++)
        {
            ...
            {
                std::cout<<" Estimate: "<<a_temp[i*B+j]<<"\n";
                std::cout<<" Real: "<<code_messgae[i*B+j]<<"\n";
                std::cout<<" id_count: "<<id_cout<<"\n";
                id_count++;
            }
        }
    if(id_count == B)
