...
void AMP::gen_sparse_code()
{
    ...
    const data_t alpha = 2 * ch_capacity; // Cynthia paper  
    for (int i = 0; i < L; i++)
    {

        int j = dist(gen);
        this->code_messgae[i * B + j] = c * std::pow(2.0, -2 * ch_capacity*i/L)*(std::pow(2.0, -2 * ch_capacity/L)-1)/(1- std::pow(2.0, -2 * ch_capacity/L));
    }
}
/**
