...
/**
 * Method to generate a sparse superposition code with exponentially decaying power allocation 
 */
void AMP::gen_sparse_code_decay()
{
    // Generate a random index between 0 and section size
    ...
    // Iterate through the sections
    // data_t kapa = 1; // Decay rate 
    for (int i = 0; i < L; i++)
    {

        int j = dist(gen);
        code_messgae[i * B + j] = std::pow(2.0, -(i) / L);
    }
}
/**
