...
data_t AMP::compute_ser(const std::vector<data_t> &a_temp)
{
    ...
    for (int i = 0; i < L; i++) // Loop over the number of sections 
    {
        ...
        for (int j = 0; j < B; j++)
        {
            if (std::abs(a_temp[i * B + j] - code_messgae[i * B + j]) < 10E-6) 
            {
                id_count++; // Increse the count by one if the estimated element and the real element are 
            }
        }
        if (id_count == B)
        {
...
void AMP::solve(const int &t_max, const data_t &ep)
{
    ...
    t++;
    /**
     * FIXME: Removed the threshold condition add it  
     * 
     */ 
