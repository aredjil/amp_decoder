...
def main():
    ...
    for i in [1.3, 1.4, 1.45, 1.5, 1.6]: #NOTE: Declar the rate in an input file  
        ...
        # Plot the mean square error rate on the right column
        ax[1].plot(df_ser["Iter"], df_ser["MSE"], label=f"R={i}")
        ax[1].set_ylabel("MSE")
        ...
        ax[1].set_title("Section Error Rate (SER) vs Iteration")
