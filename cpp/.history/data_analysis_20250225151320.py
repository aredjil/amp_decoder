...
def main():
    ...
    for L in section_sizes:
        ...
        plt.plot(combined_df, label=f"{L}")
        plt.scatter(idx_min, min_pt, marker=".", s=200, label="Minmum value")
        plt.xlim(0, )
        plt.xticks(range(0, 30, 5))
        plt.xlabel("Iteration")
        plt.ylabel("Section Error Rate")
        plt.grid(True, which="", ls="--")
        plt.yscale("log")
    plt.legend(title="Number of Sections",
                    loc="best", fontsize='small', fancybox=True)
    plt.title(f"Section Error Rate (SER) Vs Iterations using different number of sections\n using a communication rate r = {r}")
    plt.savefig(f"./figures/sample_fig_{r}.png")
    plt.show()
