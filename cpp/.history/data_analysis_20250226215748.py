...
def main():
    ...
    # Plotting the figure for large sizes 1024 2048 4096 8192
    section_sizes = [1024, 2048, 4096, 8192]
    ax, fig = plt.subplots(figsize=(12, 6))
    for L in section_sizes:
        path = f"./data/ser_{r}/ser_{L}"
        filename = f"ser_{L}"
        mean_size = 0
        ...
        plt.xlim(0, )
        plt.xticks(range(0, mean_size+5, 5))
    ...
    plt.title(f"""Section Error Rate (SER) Vs number of iterations for different number of sections with  R = {r} and B = 4. """)
    plt.tight_layout()
    plt.savefig(f"./figures/{figname}.png", metadata={"B":"4", "R":"1.3"})
    # plt.show()
    print(f"figure saved at: ./figures/{figname}.png")
