...
T AMP<T>::compute_ser(const std::vector<T> &a_temp)
{
    ...
    for (int i = 0; i < L; i++) // Loop over the number of sections
    {
        ...
        for (int j = 0; j < B; j++)
        {
            if (std::abs(a_temp[i * B + j] - code_messgae[i * B + j]) < 10E-3) // abs here because the message and the estimate is double precision
            {
    ...
    ser_err = static_cast<double>(count) / static_cast<double>(L);
