...
T AMP<T>::compute_ser(const std::vector<T> &a_temp)
{
    ...
    T count = static_cast<T>(L);              // Intially it is assumed that all secions are wrongly reconstructed
    for (int i = 0; i < L; i++) // Loop over the number of sections
    {
        ...
        for (int j = 0; j < B; j++)
        {
            if (std::abs(a_temp[i * B + j] - code_messgae[i * B + j]) < 10E-4) // abs here because the message and the estimate is double precision
            {
        ...
        {
            count -= 1.0; // decrease the count of wrongly reconstructed sections by one
        }
    }
    ser_err = count / static_cast<double>(L);
...
void AMP<T>::solve(const int &t_max, const T &ep)
{
    ...
    while (delta > ep)
    {
        ...
        ser_err = compute_ser(a_temp); // compute the section error rate
        std::cout << t << ", " << ser_err << "\n";
#endif
        // Check covnergence
        delta = compute_dif(a_new, a_old);
        if (std::abs(delta -ep) < 10E-9)
        {
            break;
        }
