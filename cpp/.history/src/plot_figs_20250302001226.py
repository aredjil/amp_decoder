...
def main():
    ...
    path = "./figures"
    figname = "amp_perforamnce.png"
    fig, ax = plt.subplots(ncols=2, nrows=1, figsize=(16, 8))
    for i in [1.3, 1.4, 1.45, 1.6]: 
        df_ser = pd.read_csv(f"./data/data_ser_{i}_.csv")[:-1]
        df_mse = pd.read_csv(f"./data/data_mse_{i}_.csv")[:-1]
        xticks = range(0, int(df_mse["Iter"].size), 5) # Set the xticks 
        ...
        ax[1].set_title("Mean Square Error (MSE) vs Iterations")
    plt.tight_layout()
    fig.savefig(f"{path}/{figname}")
    print(f"Figure saved in: {path}/{figname}")
